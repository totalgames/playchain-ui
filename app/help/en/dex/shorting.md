...
In order to increase your exposure to PLC and offer liquidity to BitAssets, such
...
The PlayChain network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 PLC as it is traded on external exchanges.
...
The PlayChain network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/PLC).
...
market rates up to the SQP and close the position. The remaining PLC of the
...
increase, an additional amount of PLC is locked as collateral, while reducing
...
particular bitAsset to hand it over to the PlayChain network. After that, the

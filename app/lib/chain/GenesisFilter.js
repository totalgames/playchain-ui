...
try {
    ...
    console.log(
        "WARN: Will be unable to filter PLC 1.0 wallet imports, did not find assets/bts_genesiskeys_bloom.dat",
        e
    );
}

/**
    This should only be applied to a PLC 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the PLC 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    filter(account_keys, status) {
        if (!this.isAvailable()) {
            console.log("WARN: Missing bloom filter for PLC 0.9.x wallets");
                ...
                for (var a = 0; a < account_keys.length; a++) {
                    ...
                    for (
                        ...
                        if (/^GPH/.test(currentKey))
                            currentKey = "PLC" + currentKey.substring(3);
                        if (this.inGenesis(currentKey)) continue;
                        var addresses = key.addresses(currentKey, "PLC");
